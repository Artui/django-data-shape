...
def order_tables(tables: tuple[Table | Projection, ...]) -> tuple[Table | Projection, ...]:
    ...
    a :class:`~django_data_shape.declaration.projection.Projection` selects from two tables
    ...
    **An edge is a claim, and only a declaration gets to make one.** A statement
    ...
    :class:`~django_data_shape.declaration.shape.Shape` is built rather than when the build
    starts.
    """
