...
class Shape:
    ...
    :func:`~django_data_shape.databases.shape_digest.shape_digest` now does, and emitting
    ...
    A declaration is a :class:`~django_data_shape.declaration.table.Table` -- so many rows,
    distributed like this -- or a
    :class:`~django_data_shape.declaration.projection.Projection`, which has no row count of
    ...
    **The models' own constraints are read here too**, which is why the
    ...
    needs both. See
    :func:`~django_data_shape.declaration.check_constraints.check_constraints`.
    ...
    **Load order is checked here too, for the same reason.** Which declaration
    ...
    :func:`~django_data_shape.declaration.order_tables.order_tables`.
    """
    ...
    def canonical(self) -> object:
        ...
        :func:`~django_data_shape.databases.shape_digest.shape_digest` can name the table a
