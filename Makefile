.PHONY: help init test test-sqlite lint lint-fix format format-check type-check deps-bump docs-serve docs-build release-bump release-publish release-publish-prepare release-publish-finalize

help:
	@echo "Available targets:"
	@echo "  init             Sync deps (all groups) and install pre-commit hooks"
	@echo "  test             Run pytest against Postgres with coverage (100% required)"
	@echo "  test-sqlite      Run the portable half on SQLite (no coverage gate)"
...
# The portable half: the vocabulary, the generator and the invariant arithmetic.
# No coverage gate, because the loader and everything statistics-shaped is
# unreachable here by design -- see tests/conftest_settings.py.
test-sqlite:
	DATA_SHAPE_TEST_DATABASE=sqlite uv run pytest --cov-fail-under=0

lint:
	uv run ruff check .
	uv run ty check django_data_shape
...
# Release pipeline. The version lives in django_data_shape/version.py (pyproject
...
RELEASE_PACKAGE_NAME := django-data-shape
RELEASE_VERSION_FILES := django_data_shape/version.py|^__version__[^=]*= *
